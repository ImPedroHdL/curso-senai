...
function att7() {
    ...
    let genero = Number(prompt("Qual seu genero favorito de filmes e séries?\n1)Comédia\n2)Terror\n3)Ação\n4)Outros "))
    let netflix1 = document.getElementById("netflix1")

    switch (genero) {
        case 1:
            netflix.innerHTML = `${nome}, como você escolheu "Comédia", temos aqui algumas recomendações: `
            recomend1comedy.innerHTML = `"SUPERBAD: É HOJE"`
            recomend2comedy.innerHTML = `"DEADPOOL 1 E 2"`
            recomend3comedy.innerHTML = `"INATIVIDADE PARANORMAL"`
            break

        case 2:
            netflix.innerHTML = `${nome}, como você escolheu "Terror", temos aqui algumas recomendações: `
            recomend1horror.innerHTML = `"O EXORCISTA DO PAPA"`
            recomend2horror.innerHTML = `"A SAGA 'INVOCAÇÃO DO MAL'"`
            recomend3horror.innerHTML = `"A SAGA 'ANNABELE'"`
            break

        case 3:
            netflix.innerHTML = `${nome}, como você escolheu ação, temos aqui algumas recomendações: `
            recomend1action.innerHTML = `"A SAGA MISSÃO IMPLACAVEL"`
            recomend2action.innerHTML = `"TOPGUN 1 E 2"`
            recomend3action.innerHTML = `"TODOS OS VELOZES E FURIOSOS"`
            break

        case 4:
            netflix.innerHTML = `No momento essa opção está indisponível, aguarde para atualizações!`
            break

        default:
            netflix.innerHTML = `OPÇÃO INVÁLIDA!`
